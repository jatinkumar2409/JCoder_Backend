package com.example.data.impls.getSearch
...
class getSearchImpl(private val db : MongoDatabase) : getSearch {
    val posts = db.getCollection<Post>("posts")
    val users = db.getCollection<User>("users")
    override suspend fun searchPosts(query: String , cursor: FeedCursor? , userId : String): List<PostDTO> {
        val limit = 16
        val pipeline = mutableListOf<Document>()
        pipeline += Document("\$match", Document().append(
            "status", "public"
        )
            .append(
                "\$or" , listOf(
                    Document("caption", Document("\$regex", query).append("\$options", "i")),
                    Document("tags", Document("\$regex", query).append("\$options", "i"))
                )
            ))
        ...
        return db.getCollection<Document>("posts").aggregate(pipeline).toList().toPosts()
    }

    override suspend fun searchUsers(query: String): List<User>{
       try {
           val response = users.find(Filters.or(
               Filters.regex("name" , "^$query" , "i") ,
               Filters.regex("email" , "^$query" , "i")
           )).limit(16)
               .toList()
           println("The response is " +  response.joinToString(" , ") + "\n\n\n")
           return response
       }
       catch (e : Exception){
